...
async def on_startup() -> None:
    ...
    async def _run_migrations() -> None:
        async with engine.begin() as conn:
            ...
            await conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS privacy_policy_accepted_at TIMESTAMP"))
            await conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS privacy_policy_version VARCHAR(50)"))
            await conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS privacy_policy_url VARCHAR(500)"))
            ...
            await conn.execute(text("ALTER TABLE organization_members ADD COLUMN IF NOT EXISTS athlete_data_sharing_consent BOOLEAN DEFAULT FALSE"))
            await conn.execute(text("ALTER TABLE organization_members ADD COLUMN IF NOT EXISTS athlete_data_sharing_consented_at TIMESTAMP"))
            await conn.execute(text("ALTER TABLE organization_members ADD COLUMN IF NOT EXISTS athlete_data_sharing_consent_version VARCHAR(50)"))
