...
describe("interactive coverage 2", () => {
  it("DayDetailsModal - opened with all interactions", async () => {
    const { DayDetailsModal } = await import("../components/calendar/TrainingCalendarModals");
  ...
  it("BulkEditModal - opened with selection", async () => {
    const { BulkEditModal } = await import("../components/calendar/TrainingCalendarModals");
    const { container } = wrap(
      <BulkEditModal
        opened
        onClose={() => {}}
  ...
  it("WorkoutEditModal - opened with workout event", async () => {
    const { WorkoutEditModal } = await import("../components/calendar/TrainingCalendarModals");
    const { container } = wrap(
      <WorkoutEditModal
        opened
        onClose={() => {}}
  ...
  it("SeasonPlannerDrawer - opened with all clicks", async () => {
    const { default: SeasonPlannerDrawer } = await import(
      "../components/planner/SeasonPlannerDrawer"
    );
    const me = {
      id: 1,
      email: "a@b.c",
  ...
  it("DashboardAthleteProfileTab - inputs and submit", async () => {
    const { default: DashboardAthleteProfileTab } = await import(
      "../pages/dashboard/DashboardAthleteProfileTab"
    );
    const user = {
      id: 1,
      email: "x@y.z",
  ...
  it("ContinuousCalendarGrid - click cells", async () => {
    const { default: ContinuousCalendarGrid } = await import(
      "../components/calendar/ContinuousCalendarGrid"
    );
    const { container } = wrap(
  ...
  it("SplitsTable - segmented control + annotations toggle", async () => {
    const { SplitsTable } = await import("../components/activityDetail/SplitsTable");
    const ui = palette;
    const { container } = wrap(
      <SplitsTable
        activity={
